...
echo "BUILD_CLUSTER $BUILD_CLUSTER"
echo "BUILD_CLUSTER_NAMESPACE $BUILD_CLUSTER_NAMESPACE"
...
#  echo "Setting kind poD"
#  kubectl --namespace "$BUILD_CLUSTER_NAMESPACE" run docker --image=docker:18.09.2-dind 
  #--overrides='{ "apiVersion": "apps/v1", "kind": "Pod", "spec": { "template": { "spec": {"containers": [ { "name": "docker", "image": "docker:18.09.2-dind", "securityContext": { "privileged": true } } ] } } } }'
...
if ! kubectl --namespace "$BUILD_CLUSTER_NAMESPACE" rollout status -w deployment/docker-dind; then
  ...
  cat > docker-dind-deployment.yaml << EOF
apiVersion: apps/v1
kind: Deployment
metadata:
  labels:
...
EOF
  echo "Installing Docker Server into build cluster..."
  kubectl --namespace "$BUILD_CLUSTER_NAMESPACE" apply -f docker-dind-deployment.yaml
