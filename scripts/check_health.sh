...
LIVENESS_PROBE_URL=$(kubectl get deployments --namespace ${CLUSTER_NAMESPACE} -o json | jq -r '[.items[].spec.template.spec.containers[]? | select(.image=="'"${IMAGE_REPOSITORY}:${IMAGE_TAG}"'")] | first | .livenessProbe.httpGet | ("http://" + ${IP_ADDR} + .path + ":" + .port) ')
...
READINESS_PROBE_URL=$(kubectl get deployments --namespace ${CLUSTER_NAMESPACE} -o json | jq -r '[.items[].spec.template.spec.containers[]? | select(.image=="'"${IMAGE_REPOSITORY}:${IMAGE_TAG}"'")] | first | .readinessProbe.httpGet | ("http://" + ${IP_ADDR} + .path + ":" + .port) ')
